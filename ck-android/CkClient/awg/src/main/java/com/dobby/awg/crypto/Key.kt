...
class Key private constructor(key: ByteArray) {
    ...
    override fun hashCode(): Int {
        ...
        for (i in 0 until key.size / 4) ret =
            ret xor (key[i * 4 + 0].toInt() shr 0) + (key[i * 4 + 1].toInt() shr 8) + (key[i * 4 + 2].toInt() shr 16) + (key[i * 4 + 3].toInt() shr 24)
        return ret
    }
    ...
    fun toHex(): String {
        ...
        for (i in key.indices) {
            output[i * 2] = (87 + (key[i].toInt() shr 4 and 0xf)
                    + ((((key[i].toInt() shr 4 and 0xf) - 10) shr 8) and 38.inv())).toChar()
            output[i * 2 + 1] = (87 + (key[i].toInt() and 0xf)
                    + ((((key[i].toInt() and 0xf) - 10) shr 8) and 38.inv())).toChar()
        }
        return String(output)
    }
    ...
    companion object {
        ...
        fun generatePublicKey(privateKey: Key): Key {
            ...
            Curve25519.Companion.eval(publicKey, 0, privateKey.bytes, null)
